...
// Timestamps in these sheets are inconsistent: some omit the year
// ("Thu Jun 25 12:04 PM"), others already include it ("Wed, Jul 1, 2026 2:42 PM").
// Only append 2026 when there isn't already a 4-digit year, so we never produce
// an invalid date by doubling it up.
const ASSUMED_YEAR = 2026;

export function parseTs(s) {
    const str = String(s ?? '');
    return new Date(/\d{4}/.test(str) ? str : `${str} ${ASSUMED_YEAR}`).getTime();
}
...
export function parseCsvRowsToSeries(rows) {
    ...
    const originMs = parseTs(rows[0]['Interval Start']);
    const cumulative = [
        [originMs, 0]
    ];
    const interval = [];
    ...
    rows.forEach((row) => {
        const endMs = parseTs(row['Interval End']);
...
export function normalizeRows(rows) {
    ...
    return rows.map((row) => ({
        tStart: parseTs(row['Interval Start']),
        tEnd: parseTs(row['Interval End']),
...
export function bucketByIntervalLength(rows, targetValue) {
    ...
    // If the post never reached this granularity (e.g. a post too young to have
    // any 24h rows yet), render nothing at this bucket rather than combining its
    // finer rows into partial buckets.
...
// A compact axis label ("Jul 17 9a") for dense, rotated x-axes where the full
// "Thu Jul 17 9:21 AM" would overflow. Exact values stay available on hover.
export function formatAxisDateTimeShort(value) {
    const d = new Date(value);
    const month = d.toLocaleDateString('en-US', { month: 'short' });
    const day = d.getDate();
    let hours = d.getHours();
