...
import { readPostsIndex, writePostsIndex } from './googleSheets';
...
// writeStore). They live in the "Posts" index tab of the Google Sheet — writes
// go to Google, not the local disk, so the registry works on serverless
// (Vercel) where the filesystem is read-only at runtime.
//
// data/posts.json is kept as the SEED: until the Posts tab exists, reads fall
// back to it, and the first write copies the whole list (legacy + new) into
// the tab. From then on the sheet is authoritative.
// ───────────────────────────────────────────────────────────────────────────

const LEGACY_STORE_PATH = path.join(process.cwd(), 'data', 'posts.json');
...
async function readStore() {
  try {
    ...
    if (posts) return posts; // null = Posts tab not seeded yet
  } catch {
    // fall through to the legacy file on any sheet failure
  }
